...
// CODE --------------------------------------------------
int SVDUnfoldDataSpectra_wNLO( std::string inFile_Data_dir , std::string inFile_MC_dir   , std::string inFile_MC_name, std::string baseName , 
...
//  if(debugWrite)unfmat_TH2->Write();
  ...
  if(dokRegQA){
    ...
    for(int ki=0; ki<nKregMax;ki++){
      ...
      hgen_unf_ratio[ki]->SetTitle(("Data Unf./Toy NLO Truth Truth, kReg="+std::to_string(current_kReg)+"; Jet p_{T}; ratio").c_str());
      ...
      hgen_fold_ratio[ki]->SetTitle(("Data Fold./Toy NLO Truth Truth, kReg="+std::to_string(current_kReg)+"; Jet p_{T}; ratio").c_str());
...
int main(int argc, char* argv[]){  
  ...
  if( argc!=9 ){
    ...
    std::cout<<"do ./SVDUnfoldDataSpectra_wNLO.exe <targDataDir> <targMCDir> <targMCName> <baseOutputName> <applyNPCorrs> <doJetID> <useSimpleBins> <kRegInput>"<<std::endl;
    return rStatus;  }  
  rStatus=SVDUnfoldDataSpectra_wNLO(  (const std::string)argv[1], (const std::string)argv[2], (const std::string)argv[3], (const std::string)argv[4], 
					(int)std::atoi(argv[5]),
